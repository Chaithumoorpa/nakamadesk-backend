...
def generate_invoice_pdf(sale: Sale) -> str:
    ...
    elements.append(Paragraph(f"<b>Date:</b> {sale.invoice_date.strftime('%Y-%m-%d')}", info_style))
    ...
    # --- Table Data ---
    data = [
        ["Item Name", "Qty", "Price", "CGST", "SGST", "Total"]
    ]
    ...
    elements.append(Paragraph(f"<b>CGST Total:</b> Rs. {total_cgst:.2f}", right_align_style))
    elements.append(Paragraph(f"<b>SGST Total:</b> Rs. {total_sgst:.2f}", right_align_style))
    elements.append(Paragraph(f"<b>Grand Total:</b> Rs. {grand_total:.2f}", right_align_style))
